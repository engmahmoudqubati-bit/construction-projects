  ...
  try {
    const { rows } = await pool.query(
      `SELECT pp.item_id, pp.planned_qty,
              ...
              t.id AS tx_id, t.qty_delivered, t.delivery_ref, t.notes,
              COALESCE(SUM(t2.qty_delivered), 0) AS total_delivered
       ...
       GROUP BY pp.item_id, pp.planned_qty, i.item_code, i.item_name, i.unit_of_measure,
                ...
                t.id, t.qty_delivered, t.delivery_ref, t.notes
    ...
    for (const e of entries) {
      ...
      const { rows } = await pool.query(
        `INSERT INTO cp_delivery_transactions
           (project_id,item_id,transaction_date,qty_delivered,delivery_ref,engineer_id,notes)
         ...
         RETURNING *`,
        [project_id, e.item_id, transaction_date, e.qty_delivered,
         e.delivery_ref || null, req.user.id, e.notes || null]
      );
      results.push(rows[0]);
    }
...
router.delete('/:id', async (req, res) => {
  try {
    const { rowCount } = await pool.query(
      'DELETE FROM cp_delivery_transactions WHERE id=$1', [req.params.id]
    );
