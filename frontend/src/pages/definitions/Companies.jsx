...
export default function Companies() {
  ...
  const columns = [
    ...
    { key:'type', label:'Type',
      render: r=><span style={{fontSize:11,fontWeight:600,padding:'3px 10px',borderRadius:20,background:r.type==='holding'?'#ede9fe':'#eff6ff',color:r.type==='holding'?'#7c3aed':'#1d4ed8'}}>{r.type==='holding'?t.companyTypeHolding:t.companyTypeOrg}</span> },
    ...
    <div>
      ...
      {viewModal && (
        <div style={{position:'fixed',inset:0,background:'rgba(15,23,42,0.45)',backdropFilter:'blur(4px)',zIndex:1000,display:'flex',alignItems:'center',justifyContent:'center',padding:20}} onClick={()=>setViewModal(null)}>
          <div style={{background:'var(--card)',borderRadius:16,boxShadow:'0 24px 60px rgba(0,0,0,0.18)',width:'100%',maxWidth:520,overflow:'hidden'}} onClick={e=>e.stopPropagation()}>
            <div style={{background:'linear-gradient(135deg,#6d28d9 0%,#7c3aed 100%)',padding:'18px 24px',display:'flex',alignItems:'center',justifyContent:'space-between'}}>
              <div>
                <div style={{fontSize:11,color:'rgba(255,255,255,0.65)',textTransform:'uppercase',letterSpacing:'0.08em'}}>Companies › View</div>
                <div style={{fontSize:17,fontWeight:700,color:'#fff'}}>{viewModal.name_en}</div>
              </div>
            ...
            <div style={{padding:24,display:'grid',gridTemplateColumns:'1fr 1fr',gap:'0 32px'}}>
              {[['Code',viewModal.company_code||'—'],['Type',viewModal.type],['Name (EN)',viewModal.name_en],['Name (AR)',viewModal.name_ar],['Tax ID',viewModal.tax_id||'—']].map(([l,v])=>(
      ...
      <Modal open={modal} onClose={()=>setModal(false)} title={editing?t.editCompany:t.addCompany} parentTitle={t.companies} size="md" onSave={handleSave} saving={saving}>
        <div className="form-row">
          <div className="form-group"><label className="form-label">{t.companyType} *</label>
          ...
          <div className="form-group"><label className="form-label">{t.companyCode}</label>
            <input className="form-control" value={form.company_code} onChange={e=>setForm(f=>({...f,company_code:e.target.value}))} /></div>
        </div>
        <div className="form-row">
          <div className="form-group"><label className="form-label">{t.companyNameEn} *</label>
            <input className="form-control" value={form.name_en} onChange={e=>setForm(f=>({...f,name_en:e.target.value}))} /></div>
          <div className="form-group"><label className="form-label">{t.companyNameAr} *</label>
            <input className="form-control" dir="rtl" value={form.name_ar} onChange={e=>setForm(f=>({...f,name_ar:e.target.value}))} /></div>
        </div>
        <div className="form-row">
          <div className="form-group"><label className="form-label">{t.taxId}</label>
            <input className="form-control" value={form.tax_id} onChange={e=>setForm(f=>({...f,tax_id:e.target.value}))} /></div>
          {form.type==='organization' && (
            <div className="form-group"><label className="form-label">{t.parentCompany}</label>
