...
export default function ItemClassifications() {
  ...
  async function handleSave() {
    if (!form.classification_code||!form.classification_name) return toast('Code and name required','error');
    setSaving(true);
    try {
      const payload = {...form, parent_id:form.parent_id||null, is_active:form.is_active!==false};
      if (editing) { const u=await api.updateClassification(editing.id,payload); setList(l=>l.map(x=>x.id===editing.id?u:x)); }
      else         { const c=await api.createClassification(payload); setList(l=>[...l,c]); }
  ...
  function exportCSV() {
    const csv = ['Code,Name,Name AR,Parent', ...buildTree(list).map(r=>[r.classification_code,r.classification_name,r.classification_name_ar||'',r.parent_name||''].join(','))].join('\n');
  ...
  const columns = [
    ...
    { key:'classification_name', label:'Classification Name',
      render: r=>(
        <div style={{paddingLeft:(r._level||0)*24,display:'flex',alignItems:'center',gap:5}}>
          {r._level>0 && <span style={{color:'#9ca3af',fontSize:12}}>{r._level===1?'└─':'   └─'}</span>}
          <div>
            <div style={{fontSize:13,fontWeight:r._level===0?700:600,color:'#111827'}}>{r.classification_name}</div>
            {r.classification_name_ar && <div style={{fontSize:11,color:'#9ca3af',direction:'rtl'}}>{r.classification_name_ar}</div>}
          </div>
        </div>
      )},
    { key:'level', label:'Level', style:{width:70},
      render: r=>{ const s=levelLabel[r._level||0]; return <span style={{fontSize:11,fontWeight:600,padding:'2px 8px',borderRadius:20,background:s.bg,color:s.c}}>{s.l}</span>; }},
  ...
  return (
    <div>
      <div style={{display:'flex',alignItems:'center',gap:16,marginBottom:24,flexWrap:'wrap'}}>
        ...
        <div style={{marginLeft:'auto',display:'flex',alignItems:'center',gap:8,flexWrap:'wrap'}}>
          <div style={{display:'flex',alignItems:'center',gap:8,background:'var(--card)',border:'1px solid var(--border)',borderRadius:10,padding:'8px 14px',minWidth:220}}>
            ...
            <input style={{border:'none',outline:'none',fontSize:13,color:'var(--text)',background:'none',width:'100%',fontFamily:'inherit'}} placeholder="Search..." value={searchQuery} onChange={e=>setSearchQuery(e.target.value)} />
      ...
      <DataTable columns={columns} data={buildTree(list)} loading={loading}
        filterFields={FILTER_FIELDS} filterStorageKey="classifications_filter"
      ...
      {viewPanelOpen && viewSelected.length > 0 && (() => {
        const items2 = buildTree(list).filter(r => viewSelected.includes(r.id));
        return (
          <ViewPanel
            title="Classification"
            items={items2}
            fields={r=>[['Code',r.classification_code],['Level',`Level ${(r._level||0)+1}`],['Name (EN)',r.classification_name],['Name (AR)',r.classification_name_ar||'—'],['Parent',r.parent_name||'—'],['Status',r.is_active!==false?'Active':'Inactive']]}
        ...
        <div style={{position:'fixed',inset:0,background:'rgba(15,23,42,0.45)',backdropFilter:'blur(4px)',zIndex:1000,display:'flex',alignItems:'center',justifyContent:'center',padding:20}} onClick={()=>setViewModal(null)}>
          <div style={{background:'var(--card)',borderRadius:16,width:'100%',maxWidth:480,overflow:'hidden',boxShadow:'0 24px 60px rgba(0,0,0,0.18)'}} onClick={e=>e.stopPropagation()}>
            ...
            <div style={{padding:24,display:'grid',gridTemplateColumns:'1fr 1fr',gap:'0 32px'}}>
              {[['Code',viewModal.classification_code],['Level',`Level ${(viewModal._level||0)+1}`],['Name (EN)',viewModal.classification_name],['Name (AR)',viewModal.classification_name_ar||'—'],['Parent',viewModal.parent_name||'—'],['Status',viewModal.is_active!==false?'Active':'Inactive']].map(([l,v])=>(
      ...
      <Modal open={modal} onClose={()=>setModal(false)} title={editing?t.editClassification:t.addClassification} parentTitle={t.itemClassifications} size="sm" onSave={handleSave} saving={saving}>
        <div className="form-row">
          <div className="form-group"><label className="form-label">Code *</label>
            <input className="form-control" value={form.classification_code} onChange={e=>setForm(f=>({...f,classification_code:e.target.value}))} /></div>
        ...
        <div className="form-row">
          <div className="form-group"><label className="form-label">Name (English) *</label>
            <input className="form-control" value={form.classification_name} onChange={e=>setForm(f=>({...f,classification_name:e.target.value}))} /></div>
          <div className="form-group"><label className="form-label">Name (Arabic)</label>
            <input className="form-control" dir="rtl" value={form.classification_name_ar} onChange={e=>setForm(f=>({...f,classification_name_ar:e.target.value}))} /></div>
        </div>
        <div className="form-group"><label className="form-label">Parent Classification</label>
      ...
      <Modal open={delSelModal} onClose={()=>setDelSelModal(false)} title={`Delete ${selectedRows.length} Classifications`} parentTitle={t.itemClassifications} size="sm" onSave={handleDeleteSelected} saveLabel="Delete All">
        <p>Delete <strong>{selectedRows.length}</strong> classification(s)?</p>
      </Modal>
    </div>
  );
}