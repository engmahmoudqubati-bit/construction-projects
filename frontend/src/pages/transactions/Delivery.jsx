            ...
            <table className="tx-table">
              <thead>
                <tr>
                  ...
                  <th style={{ minWidth: 110 }}>{t.qtyDelivered}</th>
                ...
                {Object.entries(grouped).map(([group, items]) => (
                  <>
                    <tr key={`g-${group}`} style={{ background: 'var(--bg2)' }}>
                      <td colSpan={9} style={{ padding: '6px 14px', fontSize: 11, fontWeight: 700, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.06em' }}>
                        {group}
                      </td>
                    </tr>
                    {items.map(row => {
                      const pct = row.planned_qty > 0
                        ? Math.min(100, (parseFloat(row.total_delivered) / parseFloat(row.planned_qty)) * 100)
                        : 0;
                        ...
                        <tr key={row.item_id} style={{ opacity: isConfirmed ? 0.85 : 1 }}>
                          ...
                          <td>
                            <div style={{ fontSize: 12, marginBottom: 3 }}>
                              {parseFloat(row.total_delivered).toFixed(3)} ({pct.toFixed(1)}%)
                            </div>
                          ...
                          <td>
                            <input type="number" min="0" step="0.001" value={row.qty_input}
                              disabled={isConfirmed}
                              onChange={e => setField(row.item_id, 'qty_input', e.target.value)} />
                          </td>
                          <td>
                            <input type="text" value={row.ref_input}
                              disabled={isConfirmed}
                              onChange={e => setField(row.item_id, 'ref_input', e.target.value)} />
                          </td>
                          <td>
                            <input type="text" value={row.notes_input}
                              disabled={isConfirmed}
                              onChange={e => setField(row.item_id, 'notes_input', e.target.value)} />
                          </td>
                          <td>
                            {row.tx_id && (
                              <span className={`badge badge-${row.tx_status || 'draft'}`}>
                                {t.txStatuses[row.tx_status] || row.tx_status}
                              </span>
                            )}
                          </td>
                        </tr>
