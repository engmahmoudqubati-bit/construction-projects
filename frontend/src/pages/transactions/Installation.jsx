...
export default function Installation() {
  ...
  async function handleSave() {
    ...
    try {
      const entries = rows
        ...
        .map(r => ({
          item_id:       r.item_id,
          qty_installed: parseFloat(r.qty_input),
  ...
  return (
    <div>
      <div className="page-header"><h1>🔧 {t.installation}</h1></div>
                ...
                {Object.entries(grouped).map(([group, items]) => (
                  <>
                    <tr key={`g-${group}`} style={{ background: 'var(--bg2)' }}>
                      <td colSpan={7} style={{ padding: '8px 14px', fontSize: 12, fontWeight: 700, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.06em' }}>
                        {group}
                      </td>
                    </tr>
                    {items.map(row => {
                      const pct = row.planned_qty > 0
                        ? Math.min(100, (parseFloat(row.total_installed) / parseFloat(row.planned_qty)) * 100)
                        : 0;
                      return (
                        <tr key={row.item_id}>
                          ...
                          <td className="progress-cell">
                            <div style={{ fontSize: 12, marginBottom: 4 }}>
                              {parseFloat(row.total_installed).toFixed(3)} ({pct.toFixed(1)}%)
                            </div>
                          ...
                          <td><input type="text" value={row.notes_input}
                            onChange={e => setField(row.item_id, 'notes_input', e.target.value)} /></td>
                        </tr>
                      );
                    })}
                  </>
